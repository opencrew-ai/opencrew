...
sign in once. The crew is your laptop, so it has to stay awake and running for the phone
to work: keep it plugged in with sleep off (on a Mac, `caffeinate -i` in a terminal, or
Battery → Options → prevent sleeping on power). Prefer your own front door? A Cloudflare
tunnel works too (`OPENCREW_TUNNEL_TOKEN` and `OPENCREW_TUNNEL_URL`, see
